...
myApp.controller("UserController", function($scope, $http, $location, $cookies) {
  ...
  $scope.loadEditInstance = function(x) {
    $location.path('/instance/edit');
  }

  $scope.editInstance = function() {
    $scope.dummyInstance.uuid_vm = "713942e4-46ed-d41b-c4ab-0eeb549a4d3f";
    $http({
        method: 'POST',
        url: 'http://localhost:8183/instance/info',
        data: $.param($scope.dummyInstance),
      ...
      .success(function(data) {
          ...
          $scope.dummyUser.tanggal = data.json[0].tanggal;
          $scope.dummyUser.deleted = data.json[0].deleted;
          $scope.dummyUser.status_pembayaran = data.json[0].status_pembayaran;
          $scope.dummyUser.uuid_vm = data.json[0].uuid_vm;
